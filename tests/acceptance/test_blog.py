...
@given(parse_regex("I wrote (?P<count>\d+) blog articles?"))
...
def article_is_displayed(browser, articles):
    # We are reading the most recent article,
    # which is also the last one created.
    article = articles[-1]

    assert browser.is_text_present(article.title)

    # Cannot use .is_text_present() to check if the article's content
    # is present, as this method strips all HTML tags before performing
    # the verification.
    assert article.content in browser.find_by_css('.article')[0].html
...
def articles_are_sorted_in_descending_order(browser, articles):
    actual = [
        article.text
        for article in browser.find_by_css('.article-list__title')]
    expected = [
        article.title for article in articles[::-1]]
